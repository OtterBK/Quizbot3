...
function initializeNodes() {
  ...
  const FFMPEG_PCM_EDGE = {
    ...
    to: nodes.get("raw" /* Raw */),
    cost: 2,
    transformer: (input, seek, to) =>  //ffmpeg에 seek이랑 to 넘기기
	{
		let args = [];
		let base_args = ["-i", typeof input === "string" ? input : "-", ...FFMPEG_PCM_ARGUMENTS];
		base_args.forEach(arg => args.push(arg));
		if(seek != undefined) 
		{
		  args.unshift('-ss', `${seek}`);
		}
		if(to != undefined) args.push('-to', `${to}`);
		return new import_prism_media.default.FFmpeg({
		  args: args
		});
	}
  };
...
function createAudioResource(input, options = {}) {
  ...
  const seek = options.seek ?? undefined; //seek option 찾기
  const to = options.to ?? undefined; //to option 찾기
  const streams = transformerPipeline.map((edge) => 
  {
	  if(edge.type.includes('ffmpeg')) //ffmpeg edge면 seek, to 넘기기
	  {
		  return edge.transform(input, seek, to);
	  }
	  return edge.transformer(input)
  });
  if (typeof input !== "string")
