...
var FFMPEG_PCM_EDGE = {
  ...
  to: getNode("raw" /* Raw */),
  cost: 2,
  transformer: (input, seek, to) => {
    let args = [];
    let base_args = typeof input === 'string' ? ['-i', input, ...FFMPEG_PCM_ARGUMENTS] : FFMPEG_PCM_ARGUMENTS;
...
if (canEnableFFmpegOptimizations()) {
  const FFMPEG_OGG_EDGE = {
    ...
    cost: 2,
		transformer: (input, seek, to) => {
      let args = [];
			let base_args = typeof input === 'string' ? ['-i', input, ...FFMPEG_OPUS_ARGUMENTS] : FFMPEG_OPUS_ARGUMENTS;
      base_args.forEach(arg => args.push(arg));
			if(seek != undefined) 
      {
        args.unshift('-i', `-`);
...
function createAudioResource(input, options = {}) {
  ...
  {
    if(edge.type.includes('ffmpeg'))
		{
			return edge.transformer(input, seek, to);
		}
		else
		{
