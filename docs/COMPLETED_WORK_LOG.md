...
## 2026-09-03 — DB 백업 구글드라이브 오프사이트 업로드 추가

`auto_script/db_script/backup_script.sh`가 로컬 덤프(7일 보관) 생성 후 rclone이 설치+설정돼 있으면
`gdrive:quizbot_backup/`에도 동일 파일을 업로드하도록 개선 — 원래 개인 로컬 장비로 주기적으로 백업을
내려받아 보관했는데, 그 장비가 꺼져있는 등 접근 불가할 때를 대비한 보완용. 여러 세대를 쌓지 않고
**항상 최신 백업 1개만** 드라이브에 남기도록(업로드 성공 직후 그 폴더의 다른 파일은 전부 삭제) 함 —
장기 아카이브가 아니라 "로컬 장비 대신 쓰는 임시 사본" 용도이기 때문. rclone 미설치/미설정 시엔 경고만
찍고 로컬 백업은 그대로 정상 진행(필수 아님, 기존 동작 그대로 유지). 서버에서 root로
`rclone config`를 1회 설정해야 함(cron이 `sudo -E bash`로 스크립트를 돌려 결국 root 권한으로 실행되기
